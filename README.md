...
## Descripción de las principales funcionalidades de RoboDK: Comunicación y Procesos.
RoboDK cuenta con una interfaz organizada en una barra superior que agrupa sus principales funcionalidades. Entre ellas se encuentran los menús “File”, que permite importar modelos CAD y proyectos; “Station”, destinado a la creación y configuración de estaciones robóticas; “Program”, para generar, editar y simular trayectorias; y “Connect”, que gestiona la conexión directa con controladores físicos. Asimismo, dispone de herramientas para definir robots, herramientas (TCP), objetos de trabajo (WObj) y sistemas de referencia, así como funciones de calibración, generación de código RAPID o INFORM y análisis de trayectorias mediante simulación 3D.

En el caso del manipulador Motoman MH6, RoboDK se comunica con el robot a través de una conexión Ethernet mediante dirección IP, utilizando el protocolo High-Speed Ethernet Server (HSE) del controlador Motoman. Este enlace permite el envío y recepción de comandos en tiempo real, sincronizando la posición y el movimiento entre el entorno virtual y el robot físico. Durante la ejecución, RoboDK traduce las trayectorias definidas en el software en instrucciones nativas del controlador (como MOVJ y MOVL), gestionando los parámetros de velocidad, aceleración, herramienta activa y sistema de coordenadas. En consecuencia, el programa actúa como un intermediario inteligente que convierte la simulación en movimientos reales, garantizando precisión en la ejecución y reduciendo la necesidad de programación manual directa en el controlador.
